...
export default function Dashboard({ user }) {
  ...
  return (
    <div style={s.page}>
      ...
      {/* ── Sidebar ── */}
      <aside style={{ ...s.sidebar, width: sidebarCollapsed ? 58 : 252 }}>
        <div style={s.sidebarHeader}>
          <button style={s.collapseBtn} onClick={() => setSidebarCollapsed(!sidebarCollapsed)} title={sidebarCollapsed ? "Expand" : "Collapse"}>
        ...
        <button style={{ ...s.newBtn, justifyContent: sidebarCollapsed ? "center" : "flex-start" }} onClick={() => { setView("home"); setQuery(""); setReport(null); }} title="New Research">
          <IconPlus size={15} color={theme.accentText} />
          {!sidebarCollapsed && <span>New Research</span>}
        </button>

        <div style={s.historySection}>
          {!sidebarCollapsed && <div style={s.histLabel}>Recent</div>}
          <div style={s.historyList}>
            {pastReports.slice(0, 25).map((r) => (
              <div
                key={r.id}
                style={{ ...s.histItem, background: hoveredHistItem === r.id ? theme.bgHover : "transparent", justifyContent: sidebarCollapsed ? "center" : "flex-start" }}
                onClick={() => { setReport(r.report); setView("report"); }}
            ...
            {pastReports.length === 0 && !sidebarCollapsed && (
        ...
        <div style={s.sidebarBottom}>
          ...
          <div style={{ ...s.userRow, justifyContent: sidebarCollapsed ? "center" : "flex-start" }}>
            <img src={user.photoURL} alt="" style={s.avatar} referrerPolicy="no-referrer" />
            {!sidebarCollapsed && <span style={s.userName}>{user.displayName?.split(" ")[0]}</span>}
          </div>

          <button style={{ ...s.signOutBtn, justifyContent: sidebarCollapsed ? "center" : "flex-start" }} onClick={() => signOut(auth)} title="Sign out">
      ...
      {/* ── Main ── */}
      <main style={{ ...s.main, marginLeft: sidebarCollapsed ? 58 : 252 }}>
        {view === "home" && (
          <div style={s.center}>
...
function getStyles(t, isDark) {
  return {
    ...
    /* Main */
    main: {
      flex: 1, padding: "34px clamp(18px, 3.6vw, 44px)",
      overflowY: "auto", minHeight: "100vh",
      transition: "margin-left 0.2s ease",
    },
    center: { maxWidth: 980, width: "100%", margin: "24px auto 0" },
    heading: {
      fontSize: 42, fontWeight: 750, color: t.textPrimary,
      marginBottom: 8, textAlign: "center", letterSpacing: "-0.5px", lineHeight: 1.2,
    },
    subheading: {
      color: isDark ? t.textTertiary : t.textSecondary,
      fontSize: 18,
      textAlign: "center", marginBottom: 34, fontWeight: 500,
    },
    error: {
    ...
    input: {
      ...
      padding: "14px 0", color: t.inputText, fontSize: 16,
    ...
    pipelineStep: {
      flex: "1 1 90px", background: t.bgElevated,
    ...
    trendTitle: { color: t.textPrimary, fontSize: 20, fontWeight: 700, marginBottom: 5, lineHeight: 1.45 },
    trendMeta: { color: isDark ? t.textMuted : t.textSecondary, fontSize: 13, marginBottom: 7 },
    trendSummary: { color: isDark ? t.textTertiary : t.textSecondary, fontSize: 15, lineHeight: 1.6, marginBottom: 8 },
    trendLink: { color: isDark ? t.textMuted : t.textSecondary, fontSize: 13, display: "flex", alignItems: "center", gap: 4 },

    researchWrap: { maxWidth: 560, margin: "50px auto 0" },
    researchHeader: { display: "flex", alignItems: "center", gap: 10, marginBottom: 24 },
    queryLabel: { color: t.textTertiary, fontSize: 17, fontStyle: "italic", margin: 0, fontWeight: 400 },
